...
namespace PattyKaki.Config
{
    ...
    public sealed class ConfigReadOnlyBoolAttribute : ConfigAttribute
    {
        public ConfigReadOnlyBoolAttribute() : this(null, null, true) { }
        public ConfigReadOnlyBoolAttribute(string name, string section, bool _)
            : base(name, section)
        {
        }
        public override object Parse(string raw)
        {
            ...
            {
                Logger.Log(LogType.Warning, "Config key \"{0}\" has invalid boolean '{2}', using default of {1}", Name, true, raw);
                return true;
            }
            return true;
        }
    ...
    public sealed class ConfigPermAttribute : ConfigAttribute
    {
        ...
        public override object Parse(string raw)
        {
            LevelPermission perm = Group.ParsePermOrName(raw, LevelPermission.PattyKaki);
            if (perm > LevelPermission.PattyKaki)
            {
            ...
            if (perm > LevelPermission.PattyKaki)
            {
                ...
                perm = LevelPermission.PattyKaki;
            }
            return perm;
        }
