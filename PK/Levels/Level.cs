...
using PattyKaki.Bots;
using PattyKaki.DB;
using PattyKaki.Events.LevelEvents;
using PattyKaki.Levels.IO;
...
namespace PattyKaki 
{
    ...
    {        
        ...
        public bool CanJoin(Player p) {
            if (p.IsPK || this == Server.mainLevel) return true;
            
            bool skip = p.summonedMap != null && p.summonedMap.CaselessEq(name);
            LevelPermission plRank = skip ? LevelPermission.PattyKaki : p.Rank;
        ...
        public static void LoadMetadata(Level lvl) {
            try {
                ...
                if (lvl.Config.Load(propsPath)) {
                    ...
                    Logger.Log(LogType.PKMessage, ".properties file for level {0} was not found.", lvl.MapName);
                }
            } catch (Exception e) {
                Logger.LogError(e);
            }
