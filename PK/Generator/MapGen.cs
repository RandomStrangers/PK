...
using PattyKaki.Generator.fCraft;
using PattyKaki.Generator.Realistic;
using PattyKaki.Generator.Classic;

namespace PattyKaki.Generator 
{
    ...
    public sealed class MapGen 
    {
        ...
        public static void SetRealmPerms(Player p, Level lvl) {
            ...
            const LevelPermission rank = LevelPermission.PattyKaki;
            lvl.BuildAccess.Whitelist(Player.PK, rank, lvl, p.name);
            lvl.VisitAccess.Whitelist(Player.PK, rank, lvl, p.name);

            Group grp = Group.Find(Server.Config.OSPerbuildDefault);
            if (grp == null) return;
            
            lvl.BuildAccess.SetMin(Player.PK, rank, lvl, grp);
        }
    }
}