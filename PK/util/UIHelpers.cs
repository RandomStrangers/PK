...
namespace PattyKaki.UI 
{
    ...
    {        
        ...
        public static void RepeatCommand() {
            if (lastCMD.Length == 0) {
                Logger.Log(LogType.Debug, "(PattyKaki): Cannot repeat command - no commands used yet.");
                return;
            }
            Logger.Log(LogType.Debug, "Repeating &T/" + lastCMD);
        ...
        public static void HandleCommand(string text) {
            ...
            if (string.IsNullOrEmpty(text)) {
                Logger.Log(LogType.Debug, "(PattyKaki): Whitespace commands are not allowed."); 
                return;
            }
            ...
            Thread thread = new Thread(
                () =>
                {
                    ...
                    {
                        Logger.LogError(ex);
                        Logger.Log(LogType.Debug, "(PattyKaki): FAILED COMMAND");
                    }
                })
            {
                Name = "MAXCMD_" + name,
