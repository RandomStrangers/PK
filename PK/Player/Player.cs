...
using PattyKaki.DB;
using PattyKaki.Drawing;
using PattyKaki.Events.EconomyEvents;
using PattyKaki.Events.EntityEvents;
using PattyKaki.Events.PlayerEvents;
using PattyKaki.Events.PlayerDBEvents;
...
namespace PattyKaki {
    sealed class PKPlayer : Player
    {
        public PKPlayer() : base("&S(&dPatty&eKaki&S)")
        {
            group = Group.PKRank;
            color = "&S";
            SuperName = "&S&dPatty&eKaki&S";
        }

        public override string FullName
        {
            get { return "&S[&dPatty&eKaki&S]"; }
        }
    ...
    public partial class Player : Entity, IDisposable {
        ...
        public bool CanSpeak() { 
            return IsPK || (!muted && !Unverified && (voice || !Server.chatmod));
        }
