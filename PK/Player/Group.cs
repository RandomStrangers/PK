...
using PattyKaki.Events.GroupEvents;
...
namespace PattyKaki
{
    ...
    public sealed class Group
    {
        ...
        public static Group PKRank = new Group(LevelPermission.PattyKaki, int.MaxValue, int.MaxValue, "&dPatty&eKaki", "&d", int.MaxValue, int.MaxValue);
        ...
        static void AddGroup(Group temp)
        {
            ...
            else if (temp.Permission > LevelPermission.PattyKaki)
            {
                Logger.Log(LogType.Warning, "Invalid permission level for rank {0}", temp.Name);
                temp.Permission = LevelPermission.PattyKaki;
            }
            else
            {
                Register(temp);
