    ...
    public static class ChatHandler
    {
        ...
        public static void HandleCommand(Player p, string cmd, string _, CommandData __)
        {
            ...
            if (cmd.ToLower() == "pony")
            {
                ...
                int used = p.Extras.GetInt("PK_PONY");
                ...
                p.Extras["PK_PONY"] = used + 1;
            }
            else if (cmd.ToLower() == "rainbowdashlikescoolthings")
            {
                p.cancelcommand = true;
                if (!MessageCmd.CanSpeak(p, cmd)) return;
                int used = p.Extras.GetInt("PK_RD");
                ...
                p.Extras["PK_RD"] = used + 1;
            }
            if (!Server.Config.MCLawlSecretCommands) return;
            if (cmd.ToLower() == "care")
            {
                p.cancelcommand = true;
                int used = p.Extras.GetInt("PK_CARE");
                ...
                p.Extras["PK_CARE"] = used + 1;
            }
            else if (cmd.ToLower() == "facepalm")
            {
                p.cancelcommand = true;
                int used = p.Extras.GetInt("PK_FACEPALM");
